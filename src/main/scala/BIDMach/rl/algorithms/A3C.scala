...
class A3C (
		...
		val parstepper:(Array[Environment], IMat) => (Array[FMat], FMat, FMat),
		val opts:A3C.Options = new A3C.Options
		) {
...
//	Initialize them by making predictions
  	...
  	val (obs0, rewards0, dones0) = parstepper(envs, VALID_ACTIONS(ractions));           // step through parallel envs
...
//    if (render): envs[0].render()
  		...
  		for (i <- 0 until ndqn) {
  			...
  			val (obs, rewards, dones) = parstepper(envs, VALID_ACTIONS(actions));           // step through parallel envs
  			times(2) = toc;
